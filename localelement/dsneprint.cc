...
int
DSNEprint::configure(Vector<String> &conf, ErrorHandler *errh)
{
...
int
DSNEprint::initialize(ErrorHandler *errh)
{
    ...
    fprintf(stderr, "npkt,dsn,last,timestamp\n");
    return 0;
}

void
DSNEprint::cleanup(CleanupStage)
{
...
#if HAVE_BATCH
PacketBatch *
DSNEprint::pull_batch(int port, unsigned max) {
    PacketBatch *batch;
    MAKE_BATCH(DSNEprint::pull(port), batch, max);
    return batch;
}
#endif

void
DSNEprint::run_timer(Timer *t)
{
    _timeout = true;
